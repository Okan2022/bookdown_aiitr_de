...
nrow(matrix_example) #Wie viele zeilen

ncol(matrix_example) #Wie viele spalten

dim(matrix_example) #wie viele dimensionen
```
...
if(note < 2) {
  print("Good Job")
} # Sie schreiben if(Testausdruck) und dann den {Körperausdruck}, also den Körperausdruck in geschweiften Klammern.
...
```{r ifelse command}
ifelse(grade <=2, "Good Job", "Life goes on") #ifelse Befehl
```
