...
uniq_werte[which.max(freq)] # Den eindeutigen Wert mit den meisten Vorkommen ermitteln
```
...
```{r, kap4_uebung_1b, eval=FALSE}
# Interquartilsabstand, Varianz und Standardabweichung
```
...
```{r, kap4_uebung_1c, eval=FALSE}
# Alle fünf Maße auf einmal
```
...
```{r, kap4_uebung_2c, eval=FALSE}
# Korrelationsmatrix
```

d\. Gestalte die Korrelationsmatrix ansprechend.

```{r, kap4_uebung_2d, eval=FALSE}
# Gestaltung der Korrelationsmatrix
```
...
```{r, kap4_uebung_3a, eval=FALSE}
# Überblick Datensatz
```

b\. Untersuche die Struktur der fehlenden Werte in `mtcars`.

```{r, kap4_uebung_3b, eval=FALSE}
# Fehlende Werte in mtcars
```

c\. Erstelle einen automatisierten EDA-Bericht für `mtcars`!

```{r, kap4_uebung_3c, eval=FALSE}
# Automatisierter Bericht mit EDA
```
