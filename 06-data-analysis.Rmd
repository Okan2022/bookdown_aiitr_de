...
Wir müssen lediglich x-Werte einsetzen und erhalten gemäß unserem Modell eine Vorhersage des y-Werts $\hat{y_i}$. Wir können das für alle x-Werte in unserem Datensatz tun und eine Spalte mit allen vorhergesagten Werten erhalten – nennen wir sie `y_hat`:
...
CIs$missed <- ifelse(CIs$lower_ci > true_mean | CIs$upper_ci < true_mean, "Außerhalb", "Innerhalb")
...
ggplot(data = CIs) +
  geom_pointrange(
    aes(
      x = estimates,    # Punktschätzer
      xmin = lower_ci,  # unteres KI
      xmax = upper_ci,  # oberes KI
      y = id,           # y-Achse – nur die Beobachtungsnummer
      color = missed    # Farbe variiert je nach missed-Variable
    )
  ) +
  geom_vline(
    aes(xintercept = true_mean), # Vertikale Linie beim wahren Mittelwert
  ) +
...
```{r kap6 uebung 1a, eval=FALSE}
```

b\. Berechnet $\beta_0$ und $\beta_1$ automatisch mit R.
```{r kap6 uebung 1b, eval=FALSE}
```
